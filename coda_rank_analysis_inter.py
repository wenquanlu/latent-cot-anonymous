...
the_rank = pickle.load(open("cot_weights/coda_arithmetic_the_rank_16.pkl", "rb"))
print(inter_rank)
inter_results = [0 for i in range(64)]
correct_results = [0 for i in range(64)]
the_results = [0 for i in range(64)]
...
for i in range(64):
    if i%4 == 3:
        the_block3_recurrences.append(the_average_ranks[i])
...
plot.title("Logit Lens at $R_4$")
plot.legend(fontsize=14)#, loc='upper right', bbox_to_anchor=(0.98, 0.9))
plot.tight_layout()
plot.savefig("graphs/coda_arithmetic_inter_block4.png")
