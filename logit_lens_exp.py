...
# answer: single token
def logit_lens(model, tokenizer, messages, num_steps):
    ...
    for t, hidden in enumerate(selected_hidden, start=1):
        ...
        print(logits.shape)
        top_tokens = tokenizer.batch_decode(logits.topk(k = 1, dim=-1)[1][0, -1].tolist())
        print(f"Step {t} top prediction(s): {top_tokens}")

def logit_coda_lens(model, tokenizer, messages, num_steps, coda_layer = 0):
    def capture_last_block_output(module, inp, out):
    # SandwichBlock returns (hidden_state, attn_map); grab the hidden state tensor
        print("hook called!")
        hidden_states_per_step.append(out[0].detach()) 
...
def coda_lens(model, tokenizer, messages, num_steps):
    ...
    with torch.no_grad():
        for i in range(1, num_steps + 1):
            ...
            topk_ids = logits.topk(k=1).indices[0]
