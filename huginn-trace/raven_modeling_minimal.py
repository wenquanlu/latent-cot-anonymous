...
from shared_store import intermediate_coda_token, current_pred_id, top_token_rank, intermediate_result_token, intermediate_token_rank, the_token, the_token_rank
...
class RavenForCausalLM(RavenPreTrainedModel, GenerationMixin):
    ...
    def core_block_forward(
        ...
        for idx, block in enumerate(self.transformer.core_block, start=1):
            ...
            rank_inter1 = (sorted_indices == intermediate_result_token[0]).nonzero(as_tuple=False).item()
            rank_inter2 = (sorted_indices == intermediate_result_token[1]).nonzero(as_tuple=False).item()
            rank_the = (sorted_indices == the_token[0]).nonzero(as_tuple=False).item()
            rank_intern_min = min(rank_inter1, rank_inter2)
            intermediate_token_rank.append(rank_intern_min + 1)
            the_token_rank.append(rank_the + 1)
